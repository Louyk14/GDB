...
VF3StateBound::VF3StateBound(MemoryGraph *ag1, MemoryGraph *ag2, Match* m, int nclass, int* order)
{
	...
	if (predecessors[curr_n1] != -1)
	{
		...
		case NODE_DIR_OUT:
			if ((*nodeArea)[curr_n1] != NULL)
			{
				...
				unordered_map<int,vector<int>>* ins = &(*g2->inedges)[pred_pair];
				...
				while (last_candidate_index < pred_set_size)
				{
					...
					{
						bool takeover = true;
						for (const auto& v : gm->pIETreeS[cur])
						{
						...
						int maxlevel = -1;
						for (const auto& v : gm->pIETreeL[cur])
						{
							if (core_1[v] != -1 && ((core_1[v] > prev_n2)))
							{
								takeover = false;
			...
			else
			{
				...
				while (last_candidate_index < pred_set_size)
				{
					...
					else
					{
						...
						if (!takeover)
						{
							vector<int>* b = &data->communityNodes[comm];
							last_candidate_index = upper_bound(b->begin() + last_candidate_index, b->end(), maxlevel) - b->begin();
							//last_candidate_index = upper_bound(C.begin() + rank[pos], C.end(), maxlevel) - C.begin() - 1;
							continue;
						}
...
bool VF3StateBound::IsFeasiblePair(int node1, int node2)
{
	...
	for (const auto& other2 : (*g2->outedges)[node2])
	{
		...
		c_other = g2->nodeLabels[other2.first];
...
void VF3StateBound::AddPair(int node1, int node2)
{
	...
	for (const auto& other : (*g2->inedges)[node2])
	{
		...
		if (!in_2[other.first])
		{
			other_c = g2->nodeLabels[other.first];
...
{
	for (int j = 0; j < gm->pschemes.size(); ++j)
	{
		vector<int> t;
		for (int i = 0; i < n1; i++)
		{
			t.push_back(core_1[(*gm->pschemes[j])[i + 1]]);			
		}
		//gm->logger << endl;
		vvans->push_back(t);
	}
